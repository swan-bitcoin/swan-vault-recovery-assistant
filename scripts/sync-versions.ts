...
const main = () => {
  ...
  if (!['major', 'minor', 'patch'].includes(bumpType)) {
    throw new Error(`Invalid version bump type: ${bumpType}. Must be one of 'major', 'minor', or 'patch'.`)
  }
  ...
  if (!newVersion) {
    throw new Error(`Could not increment version ${currentVersion} by ${bumpType}`)
  }
  ...
  console.log('Cargo check complete. Updated src-tauri/Cargo.lock')
