...
#[tauri::command]
// #[specta::specta]
async fn fetch_balance(state: State<'_, Mutex<AppState>>) -> Result<bdk::Balance, TempuraError> {
    let state = state.lock().unwrap();

    let network = state
        .network
        .as_ref()
        .ok_or(TempuraError::NetworkError("No network set".to_string()))?
        .to_owned();
    let wallet = state
        .wallet
        .as_ref()
        .ok_or(TempuraError::ClientError("No wallet set".to_string()))?;
