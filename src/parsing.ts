...
export function getDeviceMessage(val: unknown): string {
  ...
  let message =
    devices.length === 1 ? `Found ` : `Found ${devices.length} devices: [`;

  devices.forEach((device, index, arr) => {
    message = message.concat(`a ${device.model} device`);
    if (device.error) {
      message = message.concat(
        ` which is reporting an error: '${device.error}'`
      );
...
function parseJson(val: unknown): unknown {
  if (typeof val !== "string") {
    throw new Error(
      `Expected a JSON string response, found ${typeof val}.\nresponse: ${val}`
    );
  }
...
function parseDeviceResponse(val: unknown): DeviceResponse {
  ...
  if (!Array.isArray(parsed) || !parsed.every((item) => isDevice(item))) {
    throw new Error(
      `Invalid device list found when enumerating devices.\nresponse: ${val}`
    );
  }
  return parsed;
}
...
function parseSignResponse(val: unknown): SignResponse {
  ...
  if (typeof parsed !== "object" || !isSignResponse(parsed)) {
    throw new Error(
      `Invalid response when attempting to sign PSBT.\nresponse: ${val}`
    );
  }
  return parsed;
}
